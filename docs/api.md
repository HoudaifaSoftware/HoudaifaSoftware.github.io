...
| --- | --- | --- |
| `cabinet_pending` | Cabinet not yet activated by MediSmart. | "Votre cabinet est en attente d'activation par l'équipe MediSmart." |
| `cabinet_suspended` | Cabinet suspended. | "Votre cabinet est actuellement suspendu. Contactez le support MediSmart." |
...
| `403` | Ineligible cabinet/member. | `{ "message": "<fr>", "reason": "<code>" }` |
...
- `403` — ineligible cabinet/member: `{ "message": "<fr>", "reason": "cabinet_pending|cabinet_suspended|awaiting_approval" }` (no token is issued).
...
(`RegisterCabinetAction`). The cabinet must be activated by MediSmart before the
...
`specialization`, `wilaya { code, name }`.
