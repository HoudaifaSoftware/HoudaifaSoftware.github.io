...
class StaffIndexController extends Controller
{
    ...
    public function store(Request $request, LicenseService $licenses): RedirectResponse
    {
        ...
        if ($data['assigned_to_cabinet'] && $cabinet !== null && ! $cabinet->hasAvailableSeat()) {
            throw ValidationException::withMessages([
                'email' => 'Ce cabinet a atteint sa limite de '.\App\Models\Cabinet::MAX_SEATS.' utilisateurs.',
            ]);
        }

        DB::transaction(function () use ($data, $request, $cabinet): void {
            $assign = $data['assigned_to_cabinet'];
            $user = User::query()->create([
                'name' => $data['name'],
                'email' => $data['email'],
                'password' => Hash::make($data['password']),
                'cabinet_id' => $assign ? $cabinet?->getKey() : null,
                'cabinet_setting_id' => $assign
